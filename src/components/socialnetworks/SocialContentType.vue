<template>
  <div>
    ...
    <div class="d-flex justify-content-between">
      ...
      <button class="btn custom-button" :disabled="!selectedType" @click="next">{{$t('buttons.next', {value: 'Далее'}) }}
      </button>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    types() {
      return [
        {id: 1, name: this.$t('forms.promo', {value: 'Рекламный пост'}), component: "SocialPost"},
      ]
    }
  },
  data: () => {
  ...
  methods: {
    ...
    next() {
      this.$emit('next', 'SocialContent')
      let form = this.types.find(el => {
        return el.id === +this.selectedType
      })
      this.$store.commit('selectComponentForm', form)
      this.$store.dispatch('getSocialNetworkReleases')
    },
    back() {
      this.$emit('back', 'SocialSelect')
    }
  }
}
</script>
